...
<script>
    ...
    export default {
        ...
        created() {
            axios.get("http://localhost:8082/api/recommend/getRecommendData").then((data) => {
                ...
                data.data.forEach(item => {//剩余的数据 作为剩下的列表部分
                    item.categoryList.forEach(item1 => {//获取列表里面的核心数据
                        item1.category = item.category;//存储具体的歌单名称，每个单独的数据,保存一下这个歌单的父类数据
        ...
        methods:{
            ...
            selectItem(item){
                axios.get(`http://localhost:8082/api/recommendDetail/getRecommendDetailData/${item.id}`).then((data)=>{
