<template>
    <div class="player" v-show="playlist.length>0">
            ...
            <div class="normal-player" v-show="fullScreen"><!-- 全屏播放器 -->
                ...
                <div class="bottom">
                    <div class="progress-wrapper">
                        <span class="time time-l">{{format(currentTime)}}</span> <!--左边当前播放时间-->
                        <div class="progress-bar-wrapper">
                            <progressbar :percent="percent" @percentChange="onProgressBarChange"></progressbar>
                            <!-- 调用进度条组件 -->
                        </div>
                        <span class="time time-r">{{songData.playTime}}</span><!--右边当前播放时间-->
                    </div>
        ...
        <audio v-bind:src="songData.m4aUrl" ref="audio"></audio>
...
<script>
    ...
    import progressbar from "../base/progress-bar.vue"; // 导入进度条组件
    import progressCircle from "../base/progress-circle.vue"; //导入原型进度条组件
    ...
    export default {
        ...
        methods: {
            ...
            format(time) { //歌曲的分钟
                // eslint-disable-next-line no-console
                //console.log(time)
                let min = Math.floor(time / 60); //秒
                let second = Math.floor(time % 60); // 分钟，116%60等于56

                //对于只有一位数的, 前面补个0 , 看上去格式统一一下
                if (second < 10) {//保持分钟两位数，例如：00:00
                    second = "0" + second;
                }
                if (min < 10) {  //保持秒两位数，例如：00:00
                    min = "0" + min;
                }
                return `${min}:${second}`;
            },
            ...
            prev() {
                ...
                if (this.playlist.length === 1) {
                    ...
                    let nowIndex = this.currentIndex - 1; //当前索引值减一
                    if (nowIndex < 0) {
            ...
            togglePlaying() {
                ...
                if (this.currentLyric) {
                    this.currentLyric.togglePlay(); //当我们点击暂停是，歌词也暂停
                }
            },
            handleLyric({lineNum, txt}) {
                this.currentLineNum = lineNum;//设置当前的歌词行数,目的是为了在歌词模块里面实现高亮当前行的效果
                if (lineNum > 5) { //定义歌词的每次滚动的第五句
        ...
        watch: {
            currentSong(newSong, oldSong) {
                ...
                if (this.currentLyric) {
                    this.currentLyric.stop(); //当我们切换下一首歌的时候，清除掉上一首的信息
                }
                let data = {
                    mid: this.currentSong.songMid
                };
                axios.post("http://localhost:8082/api/songDetail/getSongDetailData", JSON.stringify(data)).then((data) => {
            ...
            songData() {
                ...
                axios.post("http://localhost:8082/api/lyric/getLyric", JSON.stringify(data)).then((data) => {
