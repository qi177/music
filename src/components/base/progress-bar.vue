<template>
    <div class="progress-bar" ref="progressBar" @click="progressClick">
        <div class="bar-inner"> <!--进度条整体的部分-->
            ...
            <div class="progress-btn-wrapper" ref="progressBtn" @touchstart.prevent="progressTouchStart" @touchmove.prevent="progressTouchMove" @touchend="progressTouchEnd"><!--进度条的按钮部分 可以用触摸事件来调整播放的进度-->
    ...
    export default{
        ...
        methods:{
            ...
            progressTouchMove(e){
                ...
                const deltaX = e.touches[0].pageX-this.touch.startX;//计算手滑动距离的差值
                const offsetWidth = Math.min(this.$refs.progressBar.clientWidth-progressBtnWidth,Math.max(0,this.touch.left+deltaX))//求得最大的偏移量，整个进度条走过的区域的最大宽度求值
            ...
            progressClick(e){ //不止可以拖动滚动条，还可以点击滚动条
                const rect = this.$refs.progressBar.getBoundingClientRect(); //获取点击时的按钮的位置参数
            ...
            _triggerPercent(){
                ...
                const percent = this.$refs.progress.clientWidth/barWidth; //用已经滚过去的进度条除以总宽度，得出比例
        ...
        watch:{
            percent(newPercent){
                if(newPercent >= 0 && !this.touch.initiated){//当前比例值发生变化时，我们重新设置一次进度条的效果
